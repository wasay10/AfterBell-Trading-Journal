...
import { X, Star, ChevronDown, Plus, Search, ImagePlus, Trash2 } from "lucide-react";
...
export function NewTradeModal({ onClose, onSaved, tradeId }: NewTradeModalProps) {
  ...
  // Screenshots: pending (new files) + existing (from edit mode)
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (valid.length < files.length) setError("Some files were skipped (images only, max 5MB each)");
    const newScreenshots = valid.map((file) => ({ file, preview: URL.createObjectURL(file) }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (pendingScreenshots.length > 0) {
      try {
        screenshotUrls = await Promise.all(pendingScreenshots.map((s) => readFileAsBase64(s.file)));
      } catch {
    ...
    <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4">
      <div className="bg-[#111827] border border-white/[0.08] rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-6">
          ...
          {/* ── Rating ─────────────────────────────────────── */}
          <section>
            <h3 className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-[0.5px] mb-3">Rating</h3>
            {userSettings.ratingStyle === "STARS" ? (
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} type="button" onClick={() => setRating(rating === star ? 0 : star)} className="transition-transform hover:scale-110">
              ...
              <div className="flex flex-wrap gap-2">
                {LETTER_GRADES.map((g) => (
                  <button key={g} type="button" onClick={() => setLetterRating(letterRating === g ? "" : g)}
                    className={cn("px-3 py-1.5 rounded-lg text-sm font-bold border transition-all",
                      letterRating === g ? "bg-[#06B6D4]/15 border-[#06B6D4]/50 text-[#06B6D4]"
                      : "bg-[#0A0E12] border-white/10 text-[#64748B] hover:border-white/20")}>
                    {g}
                  </button>
                ))}
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              ...
              {pendingNewTags.map((name) => (
                <span key={name} className="px-2.5 py-1 rounded-full text-xs font-medium bg-[#06B6D4]/15 border border-[#06B6D4]/50 text-[#06B6D4] flex items-center gap-1">
                  {name}
                  <button type="button" onClick={() => setPendingNewTags((prev) => prev.filter((t) => t !== name))} className="hover:text-[#E2E8F0]"><X className="w-3 h-3" /></button>
                </span>
              ))}
            </div>
            <div className="flex gap-2">
