                ...
                <>
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={emotionStats} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <XAxis dataKey="emotion" tick={{ fill: "#8b949e", fontSize: 10 }} tickLine={false} axisLine={false} />
                      <YAxis tick={{ fill: "#8b949e", fontSize: 10 }} tickLine={false} axisLine={false} domain={[0, 100]} tickFormatter={(v) => `${v}%`} />
                      <Tooltip content={<CustomTooltip />} />
                      <Bar dataKey="winRate" name="Win Rate" radius={[4, 4, 0, 0]}>
                        {emotionStats.map((entry, i) => (
                          <Cell key={i} fill={entry.winRate >= 50 ? "#00c853" : "#ff1744"} fillOpacity={0.85} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-[#21262d]">
                          {["Emotion", "Trades", "Win Rate", "Avg P&L"].map((h) => (
                            <th key={h} className="px-3 py-2 text-left text-[#8b949e] uppercase tracking-wider font-semibold">
                              {h}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {[...emotionStats]
                          ...
                          .map((e) => (
                            <tr key={e.emotion} className="border-b border-[#21262d] hover:bg-[#1c2128]">
                              <td className="px-3 py-2.5 font-medium text-white">{e.emotion}</td>
                              <td className="px-3 py-2.5 text-[#8b949e] font-mono">{e.totalTrades}</td>
                              <td className="px-3 py-2.5 font-mono font-semibold">
                                <span className={e.winRate >= 50 ? "text-[#00c853]" : "text-[#ff1744]"}>
                                  {e.winRate.toFixed(1)}%
                                </span>
                              </td>
                              <td className={`px-3 py-2.5 font-mono font-semibold ${e.avgPnl >= 0 ? "text-[#00c853]" : "text-[#ff1744]"}`}>
                                {formatPnl(e.avgPnl)}
                              </td>
                            </tr>
                          ))}
                ...
                <>
                        ...
                        content={({ active, payload, label }) => {
                          ...
                          return (
                            <div className="bg-[#161b22] border border-[#30363d] rounded-lg px-3 py-2 text-xs">
                              ...
                              <p className="text-[#8b949e]">Count: <span className="text-white">{d?.count}</span></p>
                              <p className="text-[#8b949e]">Total Cost: <span className="text-[#ff1744]">{d ? formatPnl(d.totalCost) : ""}</span></p>
                              <p className="text-[#8b949e]">Avg P&L: <span className={d && d.avgPnl >= 0 ? "text-[#00c853]" : "text-[#ff1744]"}>{d ? formatPnl(d.avgPnl) : ""}</span></p>
                            </div>
                          );
                        }}
                      />
                      <Bar dataKey="count" fill="#ff1744" fillOpacity={0.75} radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                  ...
                  <div className="mt-4 overflow-x-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {mistakeStats.map((m) => (
                          <tr key={m.name} className="border-b border-[#21262d] hover:bg-[#1c2128]">
                            <td className="px-3 py-2.5">
                              <span className="px-2 py-0.5 bg-[#ff1744]/20 text-[#ff1744] rounded-full">
                                {m.name}
                              </span>
                            </td>
                            <td className="px-3 py-2.5 text-[#8b949e] font-mono">{m.count}</td>
                            <td className="px-3 py-2.5 font-mono font-semibold text-[#ff1744]">
                              {formatPnl(m.totalCost)}
                            </td>
                            <td className={`px-3 py-2.5 font-mono font-semibold ${m.avgPnl >= 0 ? "text-[#00c853]" : "text-[#ff1744]"}`}>
                              {formatPnl(m.avgPnl)}
                            </td>
                          </tr>
                        ))}
