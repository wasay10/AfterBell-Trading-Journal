...
interface Trade {
  ...
  tradeConfluences: { confluence: { id: string; name: string; priority: string } }[];
...
const GRADE_BG: Record<string, string> = {
  "A+": "bg-[#10B981]/15 border-[#10B981]/30", "A": "bg-[#10B981]/15 border-[#10B981]/30",
  "B": "bg-[#06B6D4]/15 border-[#06B6D4]/30",
  "C": "bg-[#F59E0B]/15 border-[#F59E0B]/30",
  "D": "bg-[#F43F5E]/15 border-[#F43F5E]/30", "F": "bg-[#F43F5E]/15 border-[#F43F5E]/30",
};
const PRIORITY_BADGE: Record<string, string> = {
  HIGH:   "bg-[#F43F5E]/10 text-[#F43F5E] border-[#F43F5E]/30",
  MEDIUM: "bg-[#F59E0B]/10 text-[#F59E0B] border-[#F59E0B]/30",
  LOW:    "bg-white/[0.06] text-[#64748B] border-white/10",
};
                    ...
                    {group.trades.map((trade) => (
                      <Fragment key={trade.id}>
                        <tr onClick={() => setExpandedId(expandedId === trade.id ? null : trade.id)}
                          ...
                          <td className="px-3 py-3">
                            {trade.letterRating ? (
                              <span className={cn("text-xs font-black font-mono px-2 py-0.5 rounded border", GRADE_COLORS[trade.letterRating], GRADE_BG[trade.letterRating] ?? "bg-white/[0.06] border-white/10")}>
                                {trade.letterRating}
                              </span>
                            ) : trade.rating ? (
                        ...
                        {expandedId === trade.id && (
                          <tr className="bg-[#0A0E12]">
                            <td colSpan={11} className="px-6 py-5 border-b border-white/[0.05]">
                              <div className="space-y-4 text-xs">

                                {/* ── Confluences + Grade ── */}
                                {trade.tradeConfluences.length > 0 && (
                                  <div className="bg-[#111827] border border-white/[0.08] rounded-xl p-4">
                                    <div className="flex items-center justify-between mb-3">
                                      <p className="text-[11px] font-semibold text-[#06B6D4] uppercase tracking-[0.5px]">Confluences</p>
                                      {trade.letterRating && (
                                        <div className="flex items-center gap-2">
                                          <span className="text-[#64748B] text-[10px]">Auto Grade</span>
                                          <span className={cn("text-2xl font-black font-mono", GRADE_COLORS[trade.letterRating])}>
                                            {trade.letterRating}
                                          </span>
                                        </div>
                                      )}
                                    ...
                                    {(["HIGH", "MEDIUM", "LOW"] as const).map((priority) => {
                                      const cfls = trade.tradeConfluences.filter((tc) => tc.confluence.priority === priority);
                                      if (cfls.length === 0) return null;
                                      return (
                                        <div key={priority} className="mb-2 last:mb-0">
                                          <span className={cn("inline-block px-1.5 py-0.5 rounded text-[9px] font-bold border mb-1.5", PRIORITY_BADGE[priority])}>
                                            {priority}
                                          </span>
                                          <div className="flex flex-wrap gap-1.5">
                                            {cfls.map((tc) => (
                                              <span key={tc.confluence.id} className={cn("px-2 py-0.5 rounded-full text-[11px] font-medium border", PRIORITY_BADGE[priority])}>
                                                {tc.confluence.name}
                                              </span>
                                            ))}
                                          </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  {trade.tradeTags.length > 0 && (
                                    <div>
                                      <p className="text-[#64748B] mb-1.5 font-semibold uppercase tracking-wide text-[10px]">Tags / Setup</p>
                                      <div className="flex flex-wrap gap-1">
                                        {trade.tradeTags.map((tt) => (
                                          <span key={tt.tag.name} className="px-2 py-0.5 bg-[#06B6D4]/10 text-[#06B6D4] rounded-full border border-[#06B6D4]/20 text-[11px]">
                                            {tt.tag.name}
                                          </span>
                                        ))}
                                      </div>
                                  ...
                                  {trade.tradeMistakes.length > 0 && (
                                    <div>
                                      <p className="text-[#64748B] mb-1.5 font-semibold uppercase tracking-wide text-[10px]">Mistakes</p>
                                      <div className="flex flex-wrap gap-1">
                                        {trade.tradeMistakes.map((tm) => (
                                          <span key={tm.mistake.name} className="px-2 py-0.5 bg-[#F43F5E]/10 text-[#F43F5E] rounded-full border border-[#F43F5E]/20 text-[11px]">
                                            {tm.mistake.name}
                                          </span>
                                        ))}
                                      </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                  {trade.entryTime && (
                                    <div><p className="text-[#64748B] mb-0.5 text-[10px] uppercase tracking-wide">Entry Time</p><p className="text-[#E2E8F0] font-mono">{trade.entryTime}</p></div>
                                  )}
                                  {trade.drawOnLiquidity && (
                                    <div><p className="text-[#64748B] mb-0.5 text-[10px] uppercase tracking-wide">Draw on Liquidity</p><p className="text-[#06B6D4] font-mono font-semibold">{trade.drawOnLiquidity}</p></div>
                                  )}
                                  {trade.newsDriver && (
                                    <div><p className="text-[#64748B] mb-0.5 text-[10px] uppercase tracking-wide">News Driver</p><p className="text-[#E2E8F0]">{trade.newsDriver}</p></div>
                                  )}
                                  {trade.session?.name && (
                                    <div><p className="text-[#64748B] mb-0.5 text-[10px] uppercase tracking-wide">Session</p><p className="text-[#E2E8F0]">{trade.session.name}</p></div>
                                  )}
                                  {trade.psychologyBefore && (
                                    <div><p className="text-[#64748B] mb-0.5 text-[10px] uppercase tracking-wide">Mindset Before</p><p className="text-[#E2E8F0]">{trade.psychologyBefore}</p></div>
                                  )}
                                  {trade.psychologyAfter && (
                                    <div><p className="text-[#64748B] mb-0.5 text-[10px] uppercase tracking-wide">Mindset After</p><p className="text-[#E2E8F0]">{trade.psychologyAfter}</p></div>
                                  )}
                                  {trade.rr && (
                                    <div><p className="text-[#64748B] mb-0.5 text-[10px] uppercase tracking-wide">R:R</p><p className="text-[#E2E8F0] font-mono">{trade.rr.toFixed(2)}</p></div>
                                  )}
                                </div>

                                {/* ── Notes ── */}
                                {trade.notes && (
                                  <div className="bg-[#111827] border border-white/[0.08] rounded-lg px-4 py-3">
                                    <p className="text-[#64748B] mb-1 text-[10px] uppercase tracking-wide font-semibold">Notes</p>
                                    <p className="text-[#E2E8F0] leading-relaxed whitespace-pre-line">{trade.notes}</p>
                                  </div>
                                )}
                                ...
                                {trade.screenshots.length > 0 && (
                                  <div>
                                    <p className="text-[#64748B] mb-2 text-[10px] uppercase tracking-wide font-semibold">Screenshots ({trade.screenshots.length})</p>
                                    <div className="flex gap-2 flex-wrap">
                                      {trade.screenshots.map((s, i) => (
                                        <div key={i} className="rounded-lg overflow-hidden border border-white/10 w-52 aspect-video bg-[#111827]">
