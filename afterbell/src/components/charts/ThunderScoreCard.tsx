...
export function ThunderScoreCard({ thunder }: Props) {
  const isEmpty = thunder.total === 0 &&
    thunder.winPct === 0 && thunder.profitFactor === 0 &&
    thunder.avgWL === 0 && thunder.consistency === 0;
    ...
    <div className="bg-[#111827] border border-white/[0.08] rounded-xl p-4 flex flex-col">
      ...
      {isEmpty ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-3 py-8">
          <div className="w-16 h-16 rounded-full bg-[#06B6D4]/10 border border-[#06B6D4]/20 flex items-center justify-center">
            <Zap className="w-7 h-7 text-[#06B6D4]/50" />
          </div>
          <div className="text-center">
            <p className="text-sm font-semibold text-[#E2E8F0]">No score yet</p>
            <p className="text-xs text-[#64748B] mt-1">Add trades to generate your Thunder Score</p>
          </div>
        </div>
      ) : (
        <>
